...
class ConversationViewSetTestCase(APITestCase):
    ...
    def test_get_queryset(self):
        ...
        Conversation.objects.create(name=f"conv{self.user.id}_123")
        # pylint: disable=no-member
        Conversation.objects.create(name=f"conv456_{self.user.id}")
        # pylint: disable=no-member
        Conversation.objects.create(name=f"conv{self.user.id}_789")
        # pylint: disable=no-member
