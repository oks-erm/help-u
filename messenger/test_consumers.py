...
class ChatConsumerTestCase(TransactionTestCase):
    ...
    def setUp(self):
        ...
        self.url_route = {"kwargs": {
            "q": f"conv{self.user.id}_{self.user2.id}"}}
    ...
    async def test_connect(self, mock_channel_layers):
        ...
        self.assertEqual(
            ...
            {'type': "last_50_messages",
             ...
             'to_user': CustomUserSerializer(self.user2).data,
             'has_more': False}
            )
    ...
    async def test_read_messages(self, mock_channel_layers):
        ...
        notification_group_name = str(self.user.id) + "__notifications"
        communicator2 = WebsocketCommunicator(
    ...
    def test_get_receiver(self):
        ...
        chat_consumer.conversation_name = f"conv{self.user.id}_{self.user2.id}"
...
class NotificationConsumerTests(TransactionTestCase):
    ...
    async def test_connect(self, mock_channel_layers):
        ...
        unread_count = 1
        each = [[self.user2.id, unread_count]]
        expected_message = {
            "type": "unread_count",
...
#         self.assertEqual(response["each"], [[self.user2.id, 1]])
